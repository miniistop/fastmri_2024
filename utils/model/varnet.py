...
class VarNet(nn.Module):
    ...
    def __init__(
        ...
        self.cascades = nn.ModuleList(
            [VarNetBlock(NormUnetAtt(chans, pools)) for _ in range(num_cascades)]
        )
...
class VarNetBlock(nn.Module):
    ...
    def sens_expand(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        a = fastmri.complex_mul(x, sens_maps)
        return fastmri.fft2c(a)
...
#         a = self.sens_reduce(current_kspace, sens_maps)
#         b = self.model(a)
        
#         model_term = self.sens_expand(b, sens_maps)
        bool_mask = mask.to(torch.bool)
        soft_dc = (current_kspace - ref_kspace) * bool_mask.to(torch.float) * self.dc_weight
...
class NormUnetAtt(nn.Module):
    """
    Normalized attention U-Net model.
    ...
    def unnorm(
        ...
        if num_dims == 4:
            b, c, h, w = x.shape
#            c2 = c // 2  # Get the original number of channels
            ...
            b, c, h, w, _ = x.shape
#            c2 = c // 2  # Get the original number of channels
            mean = mean.view(b, c, 1, 1, 1)
            std = std.view(b, c, 1, 1, 1)
        else:
            raise ValueError("Invalid number of dimensions for input tensor x")
