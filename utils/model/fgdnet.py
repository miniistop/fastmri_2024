...
import math
import numpy as np
from fgdnet_util import initDCTKernel,initIDCTKernel
...
class FGDNet(nn.Module):
    def __init__(self, C1=64, C2=128):
        ...
        self.conv_1dt = nn.Sequential(conv_3x3(C1, C1), nn.ReLU(inplace=True), nn.BatchNorm2d(C1))
    ...
    def forward(self, target):
        ...
        out_t = self.upsample(out_t)

        weight_t = self.conv_1dt(out_t)
        ...
        # Linear Regression'
#         print(f"out_t0 : {out_t0.shape}")
#         print(f"weight_t : {weight_t.shape}")
#         print(f"out_g0 : {out_g0.shape}")
#         print(f"weight_g : {weight_g.shape}")
#         print(f"out_n0 : {out_n0.shape}")
#         print(f"weight_n : {weight_n.shape}")
        
        out_freq = out_t0 * weight_t
